...
class Queue {
    ...
    function createData(){
        ...
        src = 1;
        dst = Math.floor(Math.random() * 8) + 3;;
        curr_data = {
            nodes: nodes,
            edges: edges
        };
    }
     // algo for min. lines of metro b/w src and dst
    function minLines(src, dest){
        ...
        for(let i = 0 ;i<(metroline[cities[src-1]]).length;i++){
            queue.enqueue([metroline[cities[src-1]][i],1,""+(metroline[cities[src-1]][i])]);
            vis[metroline[cities[src-1]][i]]=1;
        }
        while (!queue.isEmpty()) {
            let t = queue.dequeue();
    ...
    solve.onclick = function () {
        ...
        const textNode1 = document.createTextNode(`${cities[src-1]}`);
        ...
        const textNode2 = document.createTextNode(`${cities[dst-1]}`);
